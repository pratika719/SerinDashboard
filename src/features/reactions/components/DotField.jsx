...
const DotField = memo(
  ({
    ...
    const containerClasses = `${isAbsolute ? "" : "w-full h-full relative"} ${className}`.trim();
    ...
    useEffect(() => {
      ...
      function doResize() {
        ...
        if (w === 0 || h === 0) {
          console.warn("DotField: Parent element has 0 width or height!", rect);
        }
        ...
        for (let i = 0; i < len; i++) {
          ...
          if (distSq < crSq && eng > 0.01) {
            ...
            if (isBulge) {
              ...
              const push =
                t * t * p.bulgeStrength * eng;

              const angle = Math.atan2(dy, dx);

              d.sx +=
                (d.ax - Math.cos(angle) * push - d.sx) *
                0.15;

              d.sy +=
                (d.ay - Math.sin(angle) * push - d.sy) *
                0.15;
            } else {
              const angle = Math.atan2(dy, dx);
